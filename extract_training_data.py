...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
      ...
      if len(state.stack) == 0:
        list_of_words = [None, None, None]
      elif len(state.stack) == 1:
        list_of_words = [state.stack[-1], None, None]
      elif len(state.stack) == 2:
        list_of_words = [state.stack[-1], state.stack[-2], None]
      else:
        list_of_words = [state.stack[-1], state.stack[-2], state.stack[-3]]

      if len(state.buffer) == 0:
        list_of_words = list_of_words + [None, None, None]
      elif len(state.buffer) == 1:
        list_of_words = list_of_words + [state.buffer[-1], None, None]
      elif len(state.buffer) == 2:
        list_of_words = list_of_words + [state.buffer[-1], state.buffer[-2], None]
      else:
        list_of_words = list_of_words + [state.buffer[-1], state.buffer[-2], state.buffer[-3]]

      for i in range(6):
        if list_of_words[i] is None:
          list_of_indexes.append(self.word_vocab['<NULL>'])
        elif list_of_words[i] == 0:
          list_of_indexes.append(self.word_vocab['<ROOT>'])
        elif pos[list_of_words[i]] == 'CD':
          list_of_indexes.append(self.word_vocab['<CD>'])
        elif pos[list_of_words[i]] == 'NNP':
          list_of_indexes.append(self.word_vocab['<NNP>'])
        else:
          temp_word = words[list_of_words[i]]
          temp_word = temp_word.lower()
          if temp_word not in self.word_vocab:
              list_of_indexes.append(self.word_vocab['<UNK>'])
          else:
              list_of_indexes.append(self.word_vocab[temp_word])

      return np.array(list_of_indexes)


    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
      tran_pair = self.output_labels[output_pair]
      tran_encoded = np.zeros(91, dtype='float32')
      tran_encoded[tran_pair] = 1
